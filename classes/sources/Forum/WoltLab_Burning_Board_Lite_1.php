...
/* ------------------------------------------------------------------ */
//	Forum Module: WoltLab Burning Board Lite 1.x
//		module version: 1.02
//		forum versions: 1.0
//		06/10/2005
/* ------------------------------------------------------------------ */
...
class EP_Dev_Forum_News_WoltLab_Burning_Board_Lite_1_Access
{
	var $SMILIES;
	var $POSTS;
	...
	function EP_Dev_Forum_News_WoltLab_Burning_Board_Lite_1_Access(&$DB_access, &$forum_conf, &$error_handle)
	{
		...
		*/
		...
		$this->LINKS['author'] = $this->CONF['url'] . "profile.php&#63;userid=";
		$this->LINKS['thread'] = $this->CONF['url'] . "thread.php&#63;threadid=";
	}
	...
	function fetch_Posts($number_to_fetch, $ids_to_fetch, $headlines_only = false)
	{
		...
		$WHERE = "boardid='" . implode("' or boardid='", $ids_to_fetch) . "'";
		
		// Pull from database
		$this->DB->query("SELECT threadid, boardid, topic, starttime, starterid, starter, replycount, views FROM " . $this->DB->prefix . "threads WHERE ". $WHERE ." ORDER BY threadid DESC LIMIT " . $number_to_fetch);
		...
		while($current_post = $this->DB->fetch_array($posts))
		{
			...
			{
				// Get text of post
				$this->DB->query("SELECT message FROM " . $this->DB->prefix . "posts WHERE threadid='" .$current_post['threadid']. "' ORDER BY postid ASC LIMIT 1");
			...
			$this->POSTS[$post_count] = array(
									"text" => $post_text,
									"title" => $current_post['topic'],
									"author_name" => $current_post['starter'],
									"author_id" => $current_post['starterid'],
									"date" => $current_post['starttime'],
									"reply_num" => $current_post['replycount'],
									"view_num" => $current_post['views'],
									"post_id" => $current_post['threadid'],
									"cat_id" => $current_post['boardid'],
									"author_url" => $this->get_Author_Link($current_post['starterid']),
	...
	function parse_Smilies(&$text)
	{
		...
		foreach($this->SMILIES as $current_smilie)
		{
			...
			if (eregi("^(http|https|ftp)://", $current_smilie['image']))
				...
				$text = str_replace($current_smilie['text'], "<img src=\"" . $this->CONF['url'] . "images" . $current_smilie['image'] . "\" border=\"0\">", $text);
		}
	}
	...
	function fetch_Author_Image($author)
	{
		...
		$this->DB->query("SELECT avatarid FROM " . $this->DB->prefix . "users WHERE userid='" . $author . "'");
		...
		{
			$this->DB->query("SELECT avatarextension FROM " . $this->DB->prefix . "avatars WHERE avatarid='" . $avatar . "'");
	...
	function fetch_Smilies()
	{
		...
		$this->DB->query("SELECT smiliepath, smiliecode FROM " . $this->DB->prefix . "smilies ORDER BY CHAR_LENGTH(smiliecode) DESC");
		...
		{
			$this->SMILIES[$smilie_count]['text'] = $smilie_data['smiliecode'];
			$this->SMILIES[$smilie_count]['image'] = str_replace("{imagefolder}", "", $smilie_data['smiliepath']);
