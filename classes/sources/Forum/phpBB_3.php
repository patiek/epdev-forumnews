...
class EP_Dev_Forum_News_phpBB_3_Access
{
	// !!!!!!!!! -------- CUSTOM USER PREFIX -------- !!!!!!!!! //
	// !!!!!!!! ----- PHP-Nuke ----- !!!!!!!!!!! //
	var $customPrefix = "";
	...
	function EP_Dev_Forum_News_phpBB_3_Access(&$DB_access, &$forum_conf, &$error_handle)
	{
	...
	function fetch_Posts($number_to_fetch, $ids_to_fetch, $headlines_only = false)
	{
		...
		while($current_post = $this->DB->fetch_array($posts))
		{
			...
			{
				// Get text of post
				$this->DB->query("SELECT bbcode_uid, post_text FROM " . $this->DB->prefix . "posts WHERE post_id='" .$current_post['topic_first_post_id']. "' ORDER BY post_id ASC LIMIT 1");
	...
	function fetch_Author_Image($author)
	{
		...
		elseif ($avatar_type == "1" || $avatar_type =="3")
		{
	...
	function fetch_Smilies()
	{
		...
		while($smilie_data = $this->DB->fetch_array())
		{
			...
			$this->SMILIES[$smilie_count]['image'] = $smilie_data['smiley_url'];
