...
/* ------------------------------------------------------------------ */
//	Forum Module: vBulletin 2.x
//		module version: 1.01
//		forum versions: 2.3.x
//		12/4/2005
/* ------------------------------------------------------------------ */


class EP_Dev_Forum_News_vBulletin_2_Access
{
	var $SMILIES;
	var $POSTS;
	...
	function EP_Dev_Forum_News_vBulletin_2_Access(&$DB_access, &$forum_conf, &$error_handle)
	{
		...
		*/
		...
		$this->LINKS['author'] = $this->CONF['url'] . "member.php&#63;action=getinfo&#38;userid=";
		$this->LINKS['thread'] = $this->CONF['url'] . "showthread.php&#63;threadid=";
	}
	...
	function fetch_Posts($number_to_fetch, $ids_to_fetch, $headlines_only = false)
	{
		...
		// Pull from database
		$this->DB->query("SELECT threadid, title, forumid, replycount, postusername, postuserid, dateline, views FROM " . $this->DB->prefix . "thread WHERE ". $WHERE ." ORDER BY threadid DESC LIMIT " . $number_to_fetch);
		...
		while($current_post = $this->DB->fetch_array($posts))
		{
			...
			{
				// Get text of post
				$this->DB->query("SELECT pagetext FROM " . $this->DB->prefix . "post WHERE threadid='" .$current_post['threadid']. "' ORDER BY postid ASC LIMIT 1");
	...
	function fetch_Author_Image($author)
	{
		...
		{
			$this->DB->query("SELECT COUNT(*) FROM " . $this->DB->prefix . "customavatar WHERE userid='" . $author . "'");

			if ($this->DB->value() != "0") // if custom avatar
				$return = $this->CONF['url'] . "avatar.php?userid=" . $author;
			else // else no avatar
				$return = NULL;
		}
